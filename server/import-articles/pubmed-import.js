...
const getData = async ctx => {
  ...
  const aa = Object.entries(pubmed_queries).map(async ([topic, query]) => {
    const { data } = await axios.get(
      `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&${query}&usehistory=y&retmode=json&mindate=${minDate}&maxdate=${dateTodayFormatted}`,
    )
