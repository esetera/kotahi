...
const ReferenceStructuring = () => {
  ...
  const referenceDetailsList = () => {
    ...
    allBlockNodes.forEach((node, pos) => {
      if (node.node.isBlock && node.node.attrs.class === 'reflist') {
        ...
        refListParagraphs.forEach((node, pos) => {
          referenceBlock.push({
        ...
        refListListItems.forEach((node, pos) => {
          referenceBlock.push({
            text: node.node.textContent,
            data_id: node.node.attrs.refId,
            valid: node.node.attrs.valid === 'true',
    ...
    // turn this off when we actually have references.
  ...
  const ReferenceList = ({ referenceDetails }) => {
    if (!referenceDetails) {
      console.log(
        'referenceDetails undefined in ReferenceList:',
        referenceDetails,
      )
      referenceDetails = []
    }
  ...
  const ReStructuredList = ({ newList }) => {
    if (!newList) {
      console.log('newlist n ReStructuredList undefined: ', newList)
      newList = []
    }
